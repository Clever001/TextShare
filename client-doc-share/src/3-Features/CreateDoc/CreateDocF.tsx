...
export default function CreateDocF(props: Props) {
  const users: string[] = [
    "Виталий Никончук", "Андрей Желудин", "Виктор Карпов", "Семен Котов",
  ]

  return <form className="create-doc-feature">
    <SectionTitle title="Создание пустого документа" />
    <div className="settings-container">
      <ValueInput type="input" keyPosition="left" label="Название" formSearchName="name"
        hasRollbackButton={false}/>
      <ValueInput type="input" keyPosition="left" label="Теги" formSearchName="tags"
        hint="первый_тег второй_тег" hasRollbackButton={false}/>
      <ValueInput type="textarea" keyPosition="left" label="Описание" formSearchName="discription"
        hasRollbackButton={false}/>
    </div>
    ...
    <CustomButton leftIconUrl={null} rightIconUrl={null} text="Создать документ"
      color="green" type="submit" target="create"/>
  </form>
}