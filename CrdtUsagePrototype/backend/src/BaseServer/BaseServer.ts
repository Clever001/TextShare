...
export class BaseServer {
  ...
  private requestNumber: bigint = BigInt(0);
  ...
  private setup() {
    this.wss.on('connection', (socket: ClientSocket) => {
      ...
      this.router.onConnect(socket);
    
      socket.on('message', (message: Buffer) => {
        this.logger.info(`Получено сообщение ${this.requestNumber}.`);
        this.requestNumber += BigInt(1);

        try {
          this.router.onMessage(message, socket);
        } catch (error) {
